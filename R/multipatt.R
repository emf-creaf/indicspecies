...
#'
multipatt_alt <- function (x, cluster, func = "IndVal.g", duleg = FALSE, restcomb=NULL,
                       ...
                       control = how(), permutations = NULL, print.perm = FALSE)
{
  ...
  # if order is 1 and function is r should select the largest magnitude association regardless of sign
  # save sign so it can be reapplied to output
  str_sign <- sign(str)
  if(!is.null(max.order)){
    if(max.order < 2 && func=="r.g"|| func=="r"){
      str <- abs(str)
    }
  }
  ...
  # get sign of max value in each row
  max_sign <- vector(length = nrow(str))
  for(i in 1:nrow(str)){max_sign[i] <- str_sign[i, wmax[i]]}
  ...
  m$index <- wmax
  m$stat <- maxstr * max_sign # reapply sign to output
